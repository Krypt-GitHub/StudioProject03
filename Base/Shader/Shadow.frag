...
uniform bool enableMT;

// Reflection
uniform bool enableReflect;
...
float getShadowEffect()
{
	...
	for(int i = 0; i < 16; ++i)
	{
		...
		if(texture2D(shadowMap, shadowCoord.xy + poissonDisk[i] / 1000.0).x < shadowCoord.z - bias)
		{
			visibility -= 0.0625;
		}
	}
...
void main()
{
	...
	if(lightEnabled == true)
	{
		...
		for (int i = 0; i < MAX_TEXTURES; ++i)
		{
			if(colorTextureEnabled[i] == true)
			{
				if (enableMT == true)
				{
					materialColor += texture2D( colorTexture[i], vec2(texCoord.x + myTime, texCoord.y + myTime) );
				}
				else
				{
	...
	else
	{
		...
		else
		{
			...
			if (enableReflect == true)
			{
				color += vec4(texture2D(colorTexture[0], vec2(R.x, R.y)).xyz, 0.5);
			}
			...
			for (int i = 0; i < MAX_TEXTURES; ++i)
			{
				if(colorTextureEnabled[i] == true)
				{
					if (enableMT == true)
					{
						color += texture2D( colorTexture[1], vec2(texCoord.x + myTime, texCoord.y + myTime));
					}
					else
					{
